...
@file    simulator_demo_minee.py
@ Original authors  Evandro de Souza, Amrinder S. Grewal
@ Edited by Sakirat Wolly
@date    2024.03.16  
@version 0.1
...
#--- 2. State Estimation with IEEE13 
Scenario = 1
...
if Scenario == 1:
    ...
    # IEEE13DSE
    TOPO_RPATH_FILE = 'SIMDSE_IEEE13/outfile2.dss'
    NWL_RPATH_FILE  = 'SIMDSE_IEEE13/IEEE13Nodeckt_NodeWithLoad.csv'
    ILPQ_RPATH_FILE = 'SIMDSE_IEEE13/IEEE13Nodeckt_InelasticLoadPQ.csv'
    DEVS_RPATH_FILE = 'SIMDSE_IEEE13/IEEE13_Devices1.csv'
...
END_TIME =  7000	#  10 secs
# END_TIME =  40000	#  40 secs
# END_TIME =  8000	#  10 secs
...
def  create_scenario( world, args ):
    ...
    if Scenario == 1:
        ...
        pktnetsim = world.start( 'PktNetSim',
            ...
            # LR-WPAN and 6LoWPAN (make sure the distance between two nodes is set accordingly)
            network         = "P2P",
            topology        = "IEEE13"  # For now only IEEE13
        )
    else:
    ...
    #--- DSE instance
        dsesim = estimator.DSESim(
            idt = 650, 
            ymat_file = 'SIMDSE_IEEE13/IEEE13ymatnew.npy', 
            devs_file = DEVS_RPATH_FILE,
            acc_period = 100, 
            max_iter = 5, 
            threshold = 0.001,
            baseS =  100e3/3,            # single phase power base
            baseV = 4.16e3/np.sqrt(3),
            baseNode = 650  ,               # single phase voltage base
            basePF = 0.99,
            se_period = 1000, # state estimation period in ms
            pseudo_loads = 'SIMDSE_IEEE13/loadPseudo133.mat',
            se_result = 'SIMDSE_IEEE13/wls_results_13.mat' # save the wls results
        )
